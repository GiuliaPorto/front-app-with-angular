    ...
    this.service.create(this.customer).subscribe({ next: () => {
      ...
      if(ex.error.errors) {
        ...
        ex.error.errors.forEach(element => {
          this.toast.error(element.message, 'Erro');  //Aparece o erro (mas não temos o devido tratamento no backend)
        });
      } else {
        this.toast.error(ex.error.message, 'Erro'); //Aparece o erro (mas não temos o devido tratamento no backend)
      }
    }});
  }
